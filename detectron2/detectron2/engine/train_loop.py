...
def create_dali_pipeline(batch_size, num_threads, device_id, data_dir):
    ...
    for prefix in files:
        img_files.append(join(data_dir, "leftImg8bit_bmp/train", prefix + "_leftImg8bit.bmp"))
        seg_files.append(join(data_dir, "gtFine/train", prefix + "_gtFine_labelTrainIds.png"))
...
#        host_memory_padding = 140544512 if decoder_device == 'mixed' else 0
        ...
        segs = fn.image_decoder(segs, device=decoder_device, output_type=types.RGB,
                                device_memory_padding=device_memory_padding,
                                host_memory_padding=host_memory_padding)

        imgs = fn.crop_mirror_normalize(imgs, device=dali_device,
                                        dtype=types.UINT8, crop=(512, 1024),
                                        output_layout="CHW")

        segs = fn.crop_mirror_normalize(segs, device=dali_device,
                                        dtype=types.UINT8, crop=(512, 1024),
                                        output_layout="CHW")
...
class SimpleTrainer(TrainerBase):
    ...
    def __init__(self, model, data_loader, optimizer):
        ...
        pipe = create_dali_pipeline(batch_size=12, num_threads=6,
                                    device_id=0, data_dir="/home/ubuntu/cityscapes")

        pipe.build()
        dali_data_loader = DALIGenericIterator(pipe, output_map=['image', 'sem_seg'], last_batch_policy=LastBatchPolicy.PARTIAL)

        self.dali_data_loader = dali_data_loader
        self._dali_data_loader_iter = iter(dali_data_loader)
    ...
    def run_step(self):
        ...
        for i in range(0, 12):
            ...
            d_data.append({'file_name': "", 'height': 1024, 'width': 2048, 'image': img, 'sem_seg': seg})
